        ...
        try:
            ...
            for i in range(0, num_devices):
                if (p.get_device_info_by_host_api_device_index(0, i).get('maxInputChannels')) > 0:
                    print(f"입력 장치 {i}: {p.get_device_info_by_host_api_device_index(0, i).get('name')}")
                if (p.get_device_info_by_host_api_device_index(0, i).get('maxOutputChannels')) > 0:
                    print(f"출력 장치 {i}: {p.get_device_info_by_host_api_device_index(0, i).get('name')}")
    ...
    def text_to_speech(self, text):
        ...
        try:
            ...
            voice = texttospeech.VoiceSelectionParams(
                ...
                name="ko-KR-Neural2-C"  # 한국어 신경망 음성 사용
            )
...
def main():
    ...
    try:
        ...
        if not args.verbose:
            # ALSA 오류 메시지 리다이렉트
            os.environ['ALSA_CARD'] = 'Generic'  # 기본 카드 설정
